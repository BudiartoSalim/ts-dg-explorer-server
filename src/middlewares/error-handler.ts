...
const errorHandler: ErrorRequestHandler = (err, req, res, next) => {
  ...
  error.errorDetail ? unhandledError(error, res) : res.status(error.status).json({ errorMessage: error.message });
};
...
function errorDef(err: unknown): IError {
  ...
  return { status: 500, message: "Internal Server Error", errorDetail: err };
}
...
function unhandledError(err: IError, res: Response) {
  ...
  res.status(err.status).json({ errorMessage: err.message, detail: err.errorDetail });
}
